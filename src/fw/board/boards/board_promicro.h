...
static const BoardConfigButton BOARD_CONFIG_BUTTON = {
  .buttons = {
    [BUTTON_ID_BACK] =
        { "Back",   { NRFX_GPIOTE_INSTANCE(0), 2, NRF_GPIO_PIN_MAP(0, 11) }, NRF_GPIO_PIN_PULLUP },
    [BUTTON_ID_UP] =
        { "Up",     { NRFX_GPIOTE_INSTANCE(0), 3, NRF_GPIO_PIN_MAP(1, 04) }, NRF_GPIO_PIN_PULLUP },
    [BUTTON_ID_SELECT] =
        { "Select", { NRFX_GPIOTE_INSTANCE(0), 4, NRF_GPIO_PIN_MAP(1, 06) }, NRF_GPIO_PIN_PULLUP },
    [BUTTON_ID_DOWN] =
...
static const BoardConfigGC9A01Display BOARD_CONFIG_DISPLAY = {
  ...
  .clk = {NRF5_GPIO_RESOURCE_EXISTS, NRF_GPIO_PIN_MAP(0,17),true}, // clock (or scl)
  .mosi = {NRF5_GPIO_RESOURCE_EXISTS, NRF_GPIO_PIN_MAP(0,20),true}, // mosi (or sda)
  .cs = {NRF5_GPIO_RESOURCE_EXISTS, NRF_GPIO_PIN_MAP(0,22),true}, // chip select
  .dc = {NRF5_GPIO_RESOURCE_EXISTS, NRF_GPIO_PIN_MAP(0,24),true}, // data/command
  .rst = {NRF5_GPIO_RESOURCE_EXISTS, NRF_GPIO_PIN_MAP(1,0),true}, //reset 
  // .extcomin is not defined here because unlike the sharp display,
  // the gc9a01 handles all of the inversion or whatever that the
  // LCD panel needs.
};
